...
get_pyladies_hosts <- function(){
  ...
  # retrieve all groups
  all_pyladies_groups <- find_groups(text = "pyladies", fields = "past_event_count, upcoming_event_count", api_key = meetup_api_key)
  ...
  pyladies_groups <- pyladies_groups[order(-pyladies_groups$past_events),]
  
  #remove groups for which event_hosts are not made public, and remove groups with 0 events
  restricted_groups <- c("pyladies-natal", "pyladies-xalapa")
  pyladies_groups <- pyladies_groups[!(pyladies_groups$past_events==0 | pyladies_groups$urlname==restricted_groups),]
  
  urlnames <- pyladies_groups$urlname 
